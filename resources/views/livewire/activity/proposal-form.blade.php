<div class="space-y-4">
    ...
    <div class="bg-surface rounded-card border border-border p-6 shadow-sm space-y-4">
        <h2 class="text-xs font-bold text-ink uppercase tracking-wider">Create Commercial Proposal for {{ $lead->name }}
        </h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            <div>
                ...
                <livewire:shared.searchable-select :model="\App\Models\ProjectUnit::class" :searchable="true"
                    placeholder="Select Unit" wire:model="projectUnitId" key="prop-unit" class="w-full mt-1" />
            </div>

            <div>
                <label class="font-bold text-ink">Base Unit Price (₹)</label>
                <input type="number" wire:model="price" step="10000"
            ...
            <div>
                ...
                <input type="number" wire:model="discount" step="5000"
                    class="w-full h-8 px-3.5 rounded-lg border border-border bg-canvas text-ink text-xs focus:outline-none focus:ring-2 focus:ring-ink transition mt-1">
            </div>
        ...
        @if($generatedSignedUrl)
            <div class="p-4 bg-canvas rounded-card border border-border space-y-2 mt-4">
                ...
                <input type="text" readonly value="{{ $generatedSignedUrl }}"
                    class="w-full text-xs font-mono p-2.5 rounded-lg border border-border bg-surface text-ink">
                <p class="text-[10px] text-muted">When the client opens this link, proposal viewed_at timestamp will be

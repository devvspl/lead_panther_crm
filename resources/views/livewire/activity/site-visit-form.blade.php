    ...
    <div class="bg-surface rounded-card border border-border p-6 shadow-sm space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            ...
            <div>
                ...
                <input type="datetime-local" wire:model="visitedAt" class="w-full p-2.5 rounded-lg border border-border bg-canvas text-ink mt-1">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea wire:model="remarks" rows="3" placeholder="Client feedback during site tour..." class="w-full p-2.5 rounded-lg border border-border bg-canvas text-ink mt-1"></textarea>
            </div>
        </div>
    ...
    @if($showStageSuggestModal)
        <div class="fixed inset-0 bg-ink/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <div class="bg-surface rounded-card border border-border p-6 shadow-xl max-w-md w-full space-y-4">
                ...
                <p class="text-xs text-muted">You logged a site visit for {{ $lead->name }}. Would you like to advance the lead's stage to <strong>Site Visit</strong>?</p>

                <div class="flex justify-end space-x-2 pt-2">
                    <button wire:click="$set('showStageSuggestModal', false)" class="text-xs text-muted hover:underline px-3 py-2">Keep Current Stage</button>

<div class="space-y-6" @if($isSyncRunning) wire:poll.2s="checkSyncProgress" @endif>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-muted">Pull production releases, review commits, safely revert codebase changes, and
                deploy with encrypted access.</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-surface rounded-card border border-border p-4 shadow-sm flex items-start justify-between">
            <div>
                ...
                <div class="text-[11px] text-muted mt-1 font-mono">Commit:
                    {{ $repoStatus['short_commit'] ?? 'unknown' }}</div>
            </div>
            <span
                class="px-2 py-0.5 text-[10px] font-bold rounded-pill bg-emerald-50 text-emerald-700 border border-emerald-200">
                Connected
            </span>
        </div>
        ...
        <div class="bg-surface rounded-card border border-border p-4 shadow-sm flex items-start justify-between">
            <div>
                ...
                <div class="text-lg font-bold text-ink mt-1">
                    {{ $repoStatus['modified_count'] ?? 0 }} Modified
                    {{ ($repoStatus['modified_count'] ?? 0) === 1 ? 'File' : 'Files' }}
                </div>
                <div
                    class="text-[11px] {{ ($repoStatus['modified_count'] ?? 0) > 0 ? 'text-amber-600 font-semibold' : 'text-emerald-600' }} mt-1">
                    {{ ($repoStatus['modified_count'] ?? 0) > 0 ? 'Uncommitted changes present' : 'Working directory clean' }}
                </div>
            </div>
            <div class="h-8 w-8 rounded-full bg-canvas border border-border flex items-center justify-center text-ink">
                <svg class="w-4 h-4 {{ ($repoStatus['modified_count'] ?? 0) > 0 ? 'text-amber-500' : 'text-emerald-500' }}"
        ...
        <div class="bg-surface rounded-card border border-border p-4 shadow-sm flex items-start justify-between">
            ...
            <span
                class="px-2 py-0.5 text-[10px] font-bold rounded-pill {{ $hasStoredToken ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200' }}">
    ...
    @if($isSyncRunning)
        <div
            class="p-4 rounded-card border border-blue-200 bg-blue-50 text-blue-900 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
            <div class="flex items-center space-x-3 text-xs">
                ...
                <div>
                    ...
                    <p class="text-blue-700 text-xs">Executing process on branch <span
                            class="font-mono font-bold">{{ $selectedBranch }}</span>. Output will refresh automatically.</p>
                </div>
            </div>
    ...
    <div class="p-1 bg-canvas rounded-xl border border-border flex items-center gap-1.5 overflow-x-auto shadow-2xs">
        <button type="button" wire:click="$set('activeTab', 'pull')"
            ...
            @if(($repoStatus['behind_count'] ?? 0) > 0)
                <span
                    class="px-1.5 py-0.2 text-[9px] rounded-pill bg-amber-500 text-white font-bold">{{ $repoStatus['behind_count'] }}</span>
            @endif
        </button>

        <button type="button" wire:click="$set('activeTab', 'push')"
            ...
            @if(($repoStatus['modified_count'] ?? 0) > 0)
                <span
                    class="px-1.5 py-0.2 text-[9px] rounded-pill bg-rose-500 text-white font-bold">{{ $repoStatus['modified_count'] }}</span>
            @endif
        </button>

        <button type="button" wire:click="$set('activeTab', 'history')"
            ...
            @if(count($commitHistory) > 0)
                <span
                    class="px-1.5 py-0.2 text-[9px] rounded-pill bg-canvas text-muted border border-border font-mono">{{ count($commitHistory) }}</span>
            @endif
        </button>

        <button type="button" wire:click="$set('activeTab', 'settings')"
        ...
        <div class="space-y-6">
            <div class="bg-surface rounded-card border border-border p-6 shadow-sm space-y-6">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
                    <div>
                        ...
                        <p class="text-xs text-muted mt-0.5">Fetches and merges the latest commits from <span
                                class="font-mono font-bold text-ink">origin/{{ $selectedBranch }}</span> into this
                            environment.</p>
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 space-y-4">
                        <div class="p-4 bg-canvas rounded-xl border border-border space-y-3 text-xs">
                            <div class="flex items-center justify-between">
                                <span class="font-bold text-ink uppercase text-[10px] tracking-wider">Latest Local
                                    Commit</span>
                                <span
                                    class="font-mono bg-surface px-2 py-0.5 rounded border border-border text-[11px] font-bold text-ink">{{ $repoStatus['short_commit'] ?? 'unknown' }}</span>
                            </div>
                            <p
                                class="text-muted leading-relaxed font-mono text-[11px] bg-surface p-2.5 rounded-lg border border-border">
                        ...
                        @if(!empty($repoStatus['recent_commits']))
                            <div class="space-y-2">
                                ...
                                <x-ui.advanced-table :columns="$localCommitCols" :rows="$repoStatus['recent_commits']"
                                    :showSearch="false" :showFilterDropdown="false" :showConfigurations="false"
                                    emptyTitle="No Recent Commits" emptyMessage="No recent local commits found." />
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="space-y-4 flex flex-col justify-between p-4 bg-canvas rounded-xl border border-border">
                        ...
                        <button type="button" wire:click="startPull" wire:loading.attr="disabled"
                            ...
                            <svg wire:loading.remove wire:target="startPull" class="w-4 h-4 text-white" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                            </svg>
                            <svg wire:loading wire:target="startPull" class="animate-spin h-4 w-4 text-white" fill="none"
                                viewBox="0 0 24 24">
                ...
                @if($lastJobResult && ($lastJobResult['action'] ?? '') === 'pull')
                    <div class="space-y-3 pt-4 border-t border-border">
                        <div class="flex items-center justify-between text-xs">
                            ...
                            <span
                                class="px-2 py-0.5 rounded-pill text-[10px] font-bold {{ ($lastJobResult['successful'] ?? false) ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200' }}">
                                {{ ($lastJobResult['successful'] ?? false) ? '✓ Succeeded' : '✕ Failed' }}
                            </span>
                        </div>

                        @if(!($lastJobResult['successful'] ?? false) && !empty($lastJobResult['friendly_error'] ?? ''))
                            <div
                                class="p-3.5 rounded-xl border border-red-200 bg-red-50 text-red-900 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs">
                                <div class="flex items-start gap-2.5">
                                    ...
                                    <div>
                                        <div class="font-bold text-red-900">Pull Requirement / Error</div>
                                        <p class="text-red-800 text-[11px] mt-0.5">{{ $lastJobResult['friendly_error'] }}</p>
                                    </div>
                                </div>
                                <button type="button" wire:click="$set('activeTab', 'settings')"
                                    class="px-3 py-1.5 rounded-lg bg-white border border-red-300 text-red-800 text-xs font-bold hover:bg-red-100 transition shrink-0 cursor-pointer shadow-2xs">
                                    Open Settings &rarr;
                                </button>
                            </div>
                        @endif
                        ...
                        <div
                            class="bg-black text-emerald-400 p-4 rounded-xl font-mono text-[11px] overflow-x-auto max-h-56 select-text whitespace-pre-wrap">
            ...
            <div class="bg-surface rounded-card border border-border p-6 shadow-sm space-y-4">
                <div>
                    ...
                    <p class="text-xs text-muted">Execute database migrations, package installations, or asset compilations
                        explicitly after reviewing pulled changes.</p>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    <button type="button" wire:click="runFollowup('migrate')" wire:loading.attr="disabled"
                    ...
                    <button type="button" wire:click="runFollowup('composer')" wire:loading.attr="disabled"
                        class="p-3.5 bg-canvas rounded-xl border border-border text-left hover:border-ink transition space-y-1.5 cursor-pointer disabled:opacity-50">
                        <div class="flex items-center justify-between">
                    ...
                    <button type="button" wire:click="runFollowup('npm')" wire:loading.attr="disabled"
                        class="p-3.5 bg-canvas rounded-xl border border-border text-left hover:border-ink transition space-y-1.5 cursor-pointer disabled:opacity-50">
                        <div class="flex items-center justify-between">
                    ...
                    <button type="button" wire:click="runFollowup('cache_all')" wire:loading.attr="disabled"
                        class="p-3.5 bg-canvas rounded-xl border border-border text-left hover:border-ink transition space-y-1.5 cursor-pointer disabled:opacity-50">
                        <div class="flex items-center justify-between">
                ...
                @if($lastFollowupResult)
                    <div
                        class="p-3 bg-black text-emerald-400 rounded-xl font-mono text-[11px] overflow-x-auto select-text whitespace-pre-wrap mt-3">
                        <div class="text-white font-bold mb-1">[Action: {{ $lastFollowupResult['action'] }}] Status:
                            {{ $lastFollowupResult['successful'] ? 'SUCCESS' : 'FAILED' }}</div>
            ...
            <div class="bg-surface rounded-card border border-border p-6 shadow-sm space-y-6">
                ...
                <div class="flex items-start space-x-3 pb-4 border-b border-border">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-muted mt-0.5">Stages modified files, creates a commit, and pushes to <span
                                class="font-mono font-bold text-rose-600">origin/{{ $selectedBranch }}</span>.</p>
                    </div>
                </div>
                ...
                @if($secretScanResult && $secretScanResult['has_secrets'])
                    <div class="p-4 rounded-xl border border-red-300 bg-red-50 text-red-900 space-y-2">
                        ...
                        <p class="text-xs">The automated pre-push scanner identified potential sensitive credentials or secret
                            files in your modified changes:</p>
                        ...
                        <div class="pt-2">
                            <label
                                class="flex items-center space-x-2 text-xs font-semibold text-red-900 cursor-pointer select-none">
                                <input type="checkbox" wire:model.live="overrideSecretBlock"
                                    class="rounded text-danger focus:ring-danger">
                                <span>I have verified these are false positives and explicitly wish to override the secret
                                    block.</span>
                            </label>
                        </div>
                    </div>
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between text-xs">
                        <span class="font-bold text-ink">Modified / Staged Files
                            ({{ count($repoStatus['modified_files'] ?? []) }})</span>
                        <button type="button" wire:click="scanForSecrets"
                    ...
                    @else
                        <div
                            class="border border-border rounded-xl overflow-hidden bg-surface max-h-48 overflow-y-auto font-mono text-xs divide-y divide-border">
                            @foreach($repoStatus['modified_files'] as $f)
                                <div class="py-2 px-3.5 flex items-center justify-between hover:bg-canvas/50">
                                    ...
                                    <span
                                        class="px-2 py-0.5 rounded text-[10px] font-bold {{ $f['flag'] === 'M' ? 'bg-amber-100 text-amber-800' : ($f['flag'] === '??' ? 'bg-emerald-100 text-emerald-800' : 'bg-canvas text-muted') }}">
                                        {{ $f['flag'] }}
                                    </span>
                                </div>
                            @endforeach
                ...
                <div class="space-y-4 pt-2">
                    <div>
                        ...
                        <input type="text" wire:model="commitMessage"
                        ...
                        @error('commitMessage') <span class="text-red-500 text-[10px] mt-0.5 block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="p-4 bg-canvas rounded-xl border border-border space-y-2.5">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                            <label class="font-bold text-xs text-ink">
                                Type <code
                                    class="px-2 py-0.5 rounded bg-surface border border-border font-mono font-bold text-rose-600 select-all">PUSH TO PRODUCTION</code>
                                to confirm:
                            </label>
                            <span class="text-[11px] text-muted">Required safety confirmation phrase</span>
                        </div>
                        <input type="text" wire:model.live="confirmPushPhrase" placeholder="PUSH TO PRODUCTION"
                        ...
                        @error('confirmPushPhrase') <span
                        class="text-red-500 text-[10px] mt-0.5 block">{{ $message }}</span> @enderror
                    </div>

                    <div class="flex items-center justify-end gap-3 pt-2">
                        <button type="button" wire:click="startPush" wire:loading.attr="disabled" @if($confirmPushPhrase !== 'PUSH TO PRODUCTION' || empty(trim($commitMessage))) disabled @endif
                            ...
                            <svg wire:loading.remove wire:target="startPush" class="w-4 h-4 text-white" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                            </svg>
                            <svg wire:loading wire:target="startPush" class="animate-spin h-4 w-4 text-white" fill="none"
                                viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4">
                                </circle>
                                <path class="opacity-75" fill="currentColor"
                            ...
                            <span wire:loading.remove wire:target="startPush">Confirm &amp; Push to
                                origin/{{ $selectedBranch }}</span>
                        ...
                        @if(!($lastJobResult['successful'] ?? false) && !empty($lastJobResult['friendly_error'] ?? ''))
                            <div
                                ...
                                <div class="flex items-start gap-2.5">
                                    ...
                                    <div>
                                        <div class="font-bold text-red-900">Push Requirement / Error</div>
                                        <p class="text-red-800 text-[11px] mt-0.5">{{ $lastJobResult['friendly_error'] }}</p>
                                    </div>
                                </div>
                                <button type="button" wire:click="$set('activeTab', 'settings')"
                ...
                <div class="bg-surface rounded-card border border-border p-6 shadow-sm space-y-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-muted">Branches automatically created and pushed prior to past revert
                                    operations. Restore anytime to undo a mistake.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border border-border rounded-xl overflow-hidden bg-canvas text-xs divide-y divide-border">
                        @foreach($backupBranches as $backup)
                            <div
                                class="p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-surface/50 transition">
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span class="font-mono font-bold text-ink">{{ $backup['name'] }}</span>
                                        <span
                                            class="font-mono text-[10px] text-primary bg-primary/10 px-1.5 py-0.5 rounded border border-primary/20">{{ $backup['short_commit'] }}</span>
                                    </div>
                                    <p class="text-[11px] text-muted mt-0.5">{{ $backup['message'] }} &bull; <span
                                            class="italic">{{ $backup['date'] }}</span></p>
                                </div>
                    ...
                    <div class="space-y-4 pt-4 border-t border-border">
                        ...
                        <div class="p-4 bg-canvas rounded-xl border border-border space-y-3">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                                <button type="button" wire:click="runFollowup('migrate')"
                                    class="px-3 py-2 rounded-lg border border-border bg-surface text-ink text-xs font-semibold hover:border-ink transition text-left cursor-pointer">
                                    1. Run Migrations
                                </button>
                                <button type="button" wire:click="runFollowup('composer')"
                                    class="px-3 py-2 rounded-lg border border-border bg-surface text-ink text-xs font-semibold hover:border-ink transition text-left cursor-pointer">
                                    2. Composer Install
                                </button>
                                <button type="button" wire:click="runFollowup('npm')"
                                    class="px-3 py-2 rounded-lg border border-border bg-surface text-ink text-xs font-semibold hover:border-ink transition text-left cursor-pointer">
                                    3. Rebuild Assets (npm)
                                </button>
                                <button type="button" wire:click="runFollowup('cache_all')"
                                    class="px-3 py-2 rounded-lg border border-border bg-surface text-ink text-xs font-semibold hover:border-ink transition text-left cursor-pointer">
                                    4. Optimize Cache
                                </button>
                            </div>
    ...
    @if($activeTab === 'settings')
        <div class="bg-surface rounded-card border border-border p-6 shadow-sm space-y-6">
            <div class="pb-4 border-b border-border">
                ...
                <p class="text-xs text-muted mt-0.5">Configure HTTPS remote repository credentials. Tokens are encrypted
                    using Laravel's encryption engine and never logged.</p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div class="md:col-span-2">
                    <label class="font-bold text-ink">Git Remote URL <span class="text-red-500">*</span></label>
                    <input type="text" wire:model="remoteUrl" placeholder="https://github.com/devvspl/lead_panther_crm.git"
                ...
                <div>
                    ...
                    <input type="text" wire:model="username" placeholder="devvspl"
                        class="w-full h-8 px-3.5 rounded-lg border border-border bg-canvas text-ink text-xs font-mono mt-1 focus:ring-2 focus:ring-primary/20 focus:border-primary">
                ...
                <div>
                    ...
                    <input type="text" wire:model="defaultBranch" placeholder="main"
                        class="w-full h-8 px-3.5 rounded-lg border border-border bg-canvas text-ink text-xs font-mono mt-1 focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    @error('defaultBranch') <span class="text-red-500 text-[10px] mt-0.5 block">{{ $message }}</span>
                    @enderror
                </div>

                <div>
                    <label class="font-bold text-ink">Git Author / Committer Name <span
                            class="text-red-500">*</span></label>
                    <input type="text" wire:model="committerName" placeholder="Lead Panther Deployer"
                        class="w-full h-8 px-3.5 rounded-lg border border-border bg-canvas text-ink text-xs mt-1 focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    <span class="text-[10px] text-muted block mt-0.5">Author name attached to Git commits and safe
                        reverts.</span>
                    @error('committerName') <span class="text-red-500 text-[10px] mt-0.5 block">{{ $message }}</span>
                    @enderror
                </div>

                <div>
                    <label class="font-bold text-ink">Git Author / Committer Email <span
                            class="text-red-500">*</span></label>
                    <input type="email" wire:model="committerEmail" placeholder="deploy@leadpanther.com"
                        class="w-full h-8 px-3.5 rounded-lg border border-border bg-canvas text-ink text-xs mt-1 focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    <span class="text-[10px] text-muted block mt-0.5">Email address used in git commit identity.</span>
                    @error('committerEmail') <span class="text-red-500 text-[10px] mt-0.5 block">{{ $message }}</span>
                    @enderror
                </div>

                <div class="md:col-span-2">
                    <div class="flex items-center justify-between">
                        <label class="font-bold text-ink">Personal Access Token (Encrypted) <span
                                class="text-red-500">*</span></label>
                        @if($hasStoredToken && !$isReplacingToken)
                            <button type="button" wire:click="$set('isReplacingToken', true)"
                                class="text-[11px] text-primary font-bold hover:underline cursor-pointer">
                                Replace Token
                            </button>
                        @endif
                    ...
                    @if($hasStoredToken && !$isReplacingToken)
                        <div class="flex items-center space-x-2 mt-1">
                            ...
                            <span
                                class="px-2.5 py-1 text-[10px] font-bold rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200 flex-shrink-0">
                                Encrypted
                            </span>
                        </div>
                    @else
                        <input type="password" wire:model="accessToken" placeholder="ghp_... or github_pat_..."
                            class="w-full h-8 px-3.5 rounded-lg border border-border bg-canvas text-ink text-xs font-mono mt-1 focus:ring-2 focus:ring-primary/20 focus:border-primary">
                        @error('accessToken') <span class="text-red-500 text-[10px] mt-0.5 block">{{ $message }}</span>
                        @enderror
                    @endif
                    <div class="mt-1.5 p-3 rounded-lg bg-canvas border border-border text-[11px] text-muted space-y-1">
                        <div class="font-bold text-ink flex items-center gap-1.5">
                            ...
                            <span>GitHub Token Requirements for Push &amp; Pull:</span>
                        </div>
                        <ul class="list-disc pl-4 space-y-0.5">
                            <li><strong>Classic Token:</strong> Ensure <code class="font-mono text-ink">repo</code> scope
                                (Full control of private repositories) is enabled.</li>
                            <li><strong>Fine-grained Token:</strong> Ensure <code
                                    class="font-mono text-ink">Repository permissions &rarr; Contents: Read and Write</code>
                                is granted for this repository.</li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            @if($connectionTestResult)
                <div
                    class="p-4 rounded-xl border {{ $connectionTestResult['successful'] ? 'bg-emerald-50 border-emerald-200 text-emerald-900' : 'bg-red-50 border-red-200 text-red-900' }} text-xs space-y-1">
            ...
            <div class="flex items-center justify-between pt-4 border-t border-border">
                <button type="button" wire:click="testConnection" wire:loading.attr="disabled"
                    ...
                    <svg wire:loading.remove wire:target="testConnection" class="w-3.5 h-3.5 text-muted" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 10V3L4 14h7v7l9-11h-7z" />
                    </svg>
    ...
    @if($showRevertModal && $selectedCommit)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
            <div
                class="bg-surface rounded-card border border-border max-w-2xl w-full p-6 shadow-2xl space-y-6 animate-in fade-in zoom-in-95">
                <!-- Modal Header -->
                <div class="flex items-start justify-between pb-4 border-b border-border">
                    <div class="flex items-center gap-3">
                        <div
                            class="p-2 bg-amber-50 dark:bg-amber-950/40 text-amber-600 rounded-lg border border-amber-200 dark:border-amber-900">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-muted">Target: <span
                                    class="font-mono font-bold text-ink">{{ $selectedCommit['short_hash'] }}</span> &bull;
                                Author: {{ $selectedCommit['author'] }}</p>
                        </div>
                    </div>
                ...
                <div
                    class="p-3.5 bg-blue-50 dark:bg-blue-950/30 rounded-xl border border-blue-200 dark:border-blue-900 text-xs text-blue-900 dark:text-blue-200 flex items-start gap-2.5">
                    ...
                    <div>
                        <span class="font-bold">Automated Safety Snapshot Guaranteed:</span> A safety backup branch <span
                            class="font-mono font-bold">backup/pre-revert-{{ date('Ymd') }}...</span> will be created at
                        current HEAD and pushed to remote before any revert runs.
                    </div>
                </div>

                <!-- Explicit Revert Strategy Options -->
                <div class="space-y-3">
                    <div class="font-bold text-xs text-ink uppercase tracking-wider">Choose Revert Strategy (Required):
                    </div>

                    <!-- OPTION A: SAFE REVERT -->
                    <label
                        class="block p-4 rounded-xl border-2 transition cursor-pointer {{ $revertStrategy === 'safe' ? 'border-primary bg-primary/5' : 'border-border bg-canvas hover:border-ink/40' }}">
                        <div class="flex items-start gap-3">
                            <input type="radio" wire:model.live="revertStrategy" value="safe"
                                class="mt-1 text-primary focus:ring-primary">
                            <div>
                                <div class="flex items-center gap-2">
                                    <span class="font-bold text-xs text-ink">Option A: Safe Revert (Recommended)</span>
                                    <span
                                        class="px-2 py-0.2 text-[9px] font-bold rounded-pill bg-emerald-100 text-emerald-800">Non-Destructive</span>
                                </div>
                                <p class="text-xs text-muted mt-1 leading-relaxed">
                                    Creates new commit(s) that undo later changes. Full history stays intact. Safe even if
                                    other team members or production systems have already pulled this branch.
                                </p>
                            </div>
                        </div>
                    </label>
                    ...
                    <!-- OPTION B: HARD RESET -->
                    <label
                        class="block p-4 rounded-xl border-2 transition cursor-pointer {{ $revertStrategy === 'hard' ? 'border-rose-500 bg-rose-500/5' : 'border-border bg-canvas hover:border-rose-500/40' }}">
                        <div class="flex items-start gap-3">
                            <input type="radio" wire:model.live="revertStrategy" value="hard"
                                class="mt-1 text-rose-600 focus:ring-rose-500">
                            <div>
                                <div class="flex items-center gap-2">
                                    <span class="font-bold text-xs text-rose-600">Option B: Hard Reset (Destructive History
                                        Rewrite)</span>
                                    <span
                                        class="px-2 py-0.2 text-[9px] font-bold rounded-pill bg-rose-100 text-rose-800">Force
                                        Push</span>
                                </div>
                                <p class="text-xs text-rose-900 dark:text-rose-300 mt-1 leading-relaxed">
                                    Permanently rewrites branch history and force-pushes. Anyone else who has pulled this
                                    branch will have conflicts. Only use this if you are certain no one else is working off
                                    this branch.
                                </p>
                            </div>
                        </div>
                ...
                @if($revertStrategy === 'hard')
                    <div class="p-4 bg-canvas rounded-xl border border-border space-y-2 text-xs">
                        <label class="font-bold text-ink block">
                            Type <code
                                class="px-2 py-0.5 rounded bg-surface border border-border font-mono font-bold text-rose-600 select-all">REVERT TO THIS COMMIT</code>
                            to confirm:
                        </label>
                        <input type="text" wire:model.live="confirmRevertPhrase" placeholder="REVERT TO THIS COMMIT"
                            class="w-full h-9 px-3.5 rounded-lg border border-border bg-surface text-ink font-mono text-xs font-bold focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500">
                        @error('confirmRevertPhrase') <span class="text-red-500 text-[10px] mt-0.5 block">{{ $message }}</span>
                        @enderror
                    </div>
                @endif
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-border">
                    <button type="button" wire:click="closeRevertModal"
                    ...
                    <button type="button" wire:click="startRevert" wire:loading.attr="disabled" @if($revertStrategy === 'hard' && $confirmRevertPhrase !== 'REVERT TO THIS COMMIT') disabled @endif
                        ...
                        <svg wire:loading wire:target="startRevert" class="animate-spin h-3.5 w-3.5 text-white" fill="none"
                            viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4">
                            </circle>
                            <path class="opacity-75" fill="currentColor"

<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-muted">Manage Real Estate Builder, Channel Partner, and Platform multi-tenant
                organizations.</p>
        </div>
    ...
    <div class="bg-surface rounded-card border border-border p-6 shadow-sm space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div>
                ...
                <input type="text" wire:model="name" placeholder="e.g. Prestige Developers Ltd"
            ...
            <div>
                ...
                <x-ui.themed-select wire:model="type" :options="['builder' => 'Builder / Real Estate Developer', 'channel_partner' => 'Channel Partner Agency', 'platform' => 'Platform Admin Org']"
                    placeholder="Organization Type" class="w-full mt-1" />
            </div>

            <div>
                <label class="font-bold text-ink">Initial Status</label>
                <x-ui.themed-select wire:model="status" :options="['active' => 'Active', 'suspended' => 'Suspended']"
                    placeholder="Initial Status" class="w-full mt-1" />
            </div>
        </div>
                        ...
                        <tr class="hover:bg-canvas/50 transition">
                            ...
                            <td class="py-3 px-4">
                                @if($org->status === 'active')
                                    <span
                                        class="px-2.5 py-0.5 text-[10px] font-bold rounded-pill bg-green-50 text-green-700 border border-green-200">ACTIVE</span>
                                @else
                                    <span
                                        class="px-2.5 py-0.5 text-[10px] font-bold rounded-pill bg-red-50 text-red-700 border border-red-200">SUSPENDED</span>
                                @endif
                            </td>

<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-muted">Configure lead assignment rules for <span class="font-bold text-ink">{{ $project->name }}</span>.</p>
        </div>
    </div>
    ...
    <div class="bg-surface rounded-card border border-border p-6 shadow-sm space-y-6">
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <!-- Round Robin Card -->
                <div 
                    wire:click="$set('ruleType', 'round_robin')" 
                ...
                >
                    ...
                    <p class="text-[11px] text-muted">Equal sequential rotation across all eligible sales executives.</p>
                </div>

                <!-- Manual Queue Card -->
                <div 
                    wire:click="$set('ruleType', 'manual')" 
                ...
                >
                    ...
                    <p class="text-[11px] text-muted">Leads sit unassigned until an Account Manager assigns them manually.</p>
                </div>

                <!-- Location-wise Card -->
                <div 
                    wire:click="$set('ruleType', 'location')" 
                ...
                <div 
                    wire:click="$set('ruleType', 'priority')" 
                ...
                <!-- Availability Card -->
                <div 
                    wire:click="$set('ruleType', 'availability')" 
                ...
                >
                    ...
                    <p class="text-[11px] text-muted">Only assign leads to executives currently marked online/active.</p>
                </div>

                <!-- Project Direct Team Card -->
                <div 
                    wire:click="$set('ruleType', 'project_wise')" 
                ...
                >
                    ...
                    <p class="text-[11px] text-muted">Assign exclusively to sales team assigned to this specific project.</p>
                </div>
            </div>
        </div>
        ...
        @if($ruleType === 'location')
            <div class="p-4 bg-canvas rounded-card border border-border space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <x-ui.button wire:click="addLocationRow" variant="secondary" class="text-[10px] px-2 py-1">+ Add City Mapping</x-ui.button>
                </div>

                @foreach($locationRows as $index => $row)
                    <div class="flex items-center space-x-3">
                        <input type="text" wire:model="locationRows.{{ $index }}.city" placeholder="City Name (e.g. Mumbai)" class="w-1/2 text-xs p-2 rounded-lg border border-border bg-surface text-ink">
                        <livewire:shared.searchable-select 
                            ...
                            key="dist-user-{{ $index }}"
                        />
                        <button wire:click="removeLocationRow({{ $index }})" class="text-danger text-xs hover:underline">Remove</button>
                    </div>
                @endforeach
            </div>

    ...
    <div class="bg-surface rounded-card border border-border p-6 shadow-sm space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            ...
            <div>
                ...
                <input type="text" wire:model="accountName" placeholder="e.g. Bandra Campaign Account" class="w-full p-2.5 rounded-lg border border-border bg-canvas text-ink mt-1">
            </div>

            <div>
                <label class="font-bold text-ink">API Key / Access Secret (Encrypted)</label>
                <input type="password" wire:model="apiSecret" placeholder="••••••••••••••••••••••••" class="w-full p-2.5 rounded-lg border border-border bg-canvas text-ink mt-1">
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-ink border-collapse">
                ...
                <tbody class="divide-y divide-border">
                        ...
                        <tr class="hover:bg-canvas/50 transition">
                            ...
                            <td class="py-3 px-4">
                                @if(isset($connectionStatus[$acc->id]))
                                    <span class="px-2.5 py-0.5 text-[10px] font-bold rounded-pill bg-green-50 text-green-700 border border-green-200">
                                        {{ $connectionStatus[$acc->id] }}
                                    </span>
                                @else
                                    <span class="px-2.5 py-0.5 text-[10px] font-bold rounded-pill bg-canvas text-muted border border-border">
                                        Untested
                                    </span>
                                @endif
                            </td>
                            <td class="py-3 px-4 text-right">
                                <button wire:click="testConnection({{ $acc->id }})" class="text-xs font-bold text-primary hover:underline">
                                    Test Connection 🔌
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="py-6 text-center text-muted">No portal account credentials configured.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
